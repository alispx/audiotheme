 ...
 * @since 2.0.0
 */
class AudioTheme_Module_Gigs_Admin {
	/**
	...
	public function load() {
		...
		// @todo Need to expose these so hooks can be modified.
		$screen_manage_gigs = new AudioTheme_Module_Gigs_Admin_Screen_ManageGigs();
		$screen_manage_gigs->load();

		$screen_edit_gig = new AudioTheme_Module_Gigs_Admin_Screen_EditGig();
		$screen_edit_gig->load();

		$screen_manage_venues = new AudioTheme_Module_Gigs_Admin_Screen_ManageVenues();
		$screen_manage_venues->load();

		$screen_edit_venue = new AudioTheme_Module_Gigs_Admin_Screen_EditVenue();
		$screen_edit_venue->load();
	}
	...
	public function register_assets() {
		wp_register_script(
			'audiotheme-gig-edit',
			AUDIOTHEME_URI . 'modules/gigs/admin/js/gig-edit.js',
		...
		wp_localize_script( 'audiotheme-gig-edit', '_audiothemeGigEditSettings', array(
			'datepickerIcon' => AUDIOTHEME_URI . 'modules/admin/images/calendar.png',
			'timeFormat'     => get_option( 'time_format' ),
		) );

		wp_register_script(
			'audiotheme-venue-edit',
			AUDIOTHEME_URI . 'modules/gigs/admin/js/venue-edit.js',
			array( 'audiotheme-admin', 'jquery-ui-autocomplete', 'post', 'underscore' ),
			AUDIOTHEME_VERSION,
			true
		);
	}
